...
begin
  ...
  DrawTextWithRect(puint16(str), CENTER_X - DrawLength(str) * 5 - 3, CENTER_Y - 150, DrawLength(str) * 10 + 6,
...
begin
  ...
  begin
    ...
    for i := 0 to BRoleAmount - 1 do
    begin
      num := Rrole[Brole[i].rnum].HeadNum;
      if HPNGIndex[num].Loaded = 0 then
        LoadOnePNGTile('resource/head', pHPic, num, HPNGIndex[num], @HPNGTile[0]);
      temp := CopyIndexSurface(HPNGIndex, num);
      BHead[i] := rotozoomSurface(temp, 0, 0.25, 1);
  ...
  begin
    ...
    begin
      ...
      begin
        LoadPNGTiles(formatfloat('resource/fight/fight000', actionnum), FPNGIndex[actionnum], FPNGTile[actionnum], 1);
      ...
      DrawShadowText(@str[1], CENTER_X - 120, CENTER_Y, ColColor(5), ColColor(7));
      UpdateAllScreen;
...
function SelectTeamMembers: integer;
var
  i, menu, max, menup, xm, ym, x, y: integer;
  ...
  begin
    ...
    DrawRectangle(screen, x + 30, CENTER_Y - 90, 150, max * 22 + 28, 0, ColColor(255), 30);
    for i := 0 to max do
      if i = menu then
      begin
        DrawShadowText(@menuString[i][1], x + 33, y + 3 + 22 * i, ColColor($66), ColColor($64));
        if ((Result and (1 shl (i - 1))) > 0) and (i > 0) and (i < max) then
          DrawShadowText(@str1[1], x + 133, y + 3 + 22 * i, ColColor($66), ColColor($64));
      end
      else
      begin
        DrawShadowText(@menuString[i][1], x + 33, y + 3 + 22 * i, ColColor($7), ColColor($5));
        if ((Result and (1 shl (i - 1))) > 0) and (i > 0) and (i < max) then
          DrawShadowText(@str1[1], x + 133, y + 3 + 22 * i, ColColor($23), ColColor($21));
      end;
    ...
    case event.type_ of
      ...
      begin
        ...
        if (event.button.button = SDL_BUTTON_LEFT) and (menu = max) and
          MouseInRegion(x, y, 150, (max + 1) * 22) then
        begin
          if Result <> 0 then
            break;
        end;
      ...
      SDL_MOUSEMOTION:
      begin
        if MouseInRegion(x, y, 150, (max + 1) * 22, xm, ym) then
        begin
          menup := menu;
          menu := (ym - 152) div 22;
          if menup <> menu then
...
function BattleMenu(bnum: integer): integer;
var
  i, p, MenuStatus, menu, max, rnum, menup, xm, ym: integer;
...
begin
  ...
  DrawTextWithRect(puint16(str), 160, 50, DrawLength(str) * 10 + 6, ColColor($21), ColColor($23), 30);
  SDL_UpdateRect2(screen, 0, 0, screen.w, screen.h);
...
begin
  ...
  begin
    ...
    case event.type_ of
      ...
      SDL_MOUSEMOTION:
      begin
        Ax := (-round(event.button.x / (RealScreen.w / screen.w)) + CENTER_X + 2 *
          round(event.button.y / (RealScreen.h / screen.h)) - 2 * CENTER_Y + 18) div 36 + Bx;
        Ay := (round(event.button.x / (RealScreen.w / screen.w)) - CENTER_X + 2 *
...
begin
  ...
  l := DrawLength(str);
  DrawTextWithRect(@str[1], CENTER_X - l * 5 - 3, CENTER_Y - 150, l * 10 + 6, ColColor($14), ColColor($16));
  SDL_UpdateRect2(screen, 0, 0, screen.w, screen.h);
  SDL_Delay(400);
  event.key.keysym.sym := 0;
...
var
  ...
  begin
    ...
    begin
      if (p = menu) and ((MenuStatus and (1 shl i) > 0)) then
      begin
        DrawShadowText(@menuString[i][1], 103, 53 + 22 * p, ColColor($64), ColColor($66));
        DrawEngShadowText(@menuEngString[i][1], 223, 53 + 22 * p, ColColor($64), ColColor($66));
        p := p + 1;
      end
      else if (p <> menu) and ((MenuStatus and (1 shl i) > 0)) then
      begin
        DrawShadowText(@menuString[i][1], 103, 53 + 22 * p, ColColor($21), ColColor($23));
...
begin
  ...
  begin
    ...
    case event.type_ of
      ...
      SDL_MOUSEMOTION:
      begin
        if MouseInRegion(100, 50, 167, max * 22 + 28, xm, ym) then
        begin
          menup := menu;
          menu := (ym - 52) div 22;
          if menu > max then
...
procedure PlayMagicAmination(bnum, enum: integer; aimMode: integer = 0; mode: integer = 0);
var
  beginpic, i, i1, i2, endpic, x, y, z, min, max, MixAlpha2, rnum: integer;
  posA, posB: TPosition;
  MixColor2: uint32;
begin
  min := 1000;
  ...
  case Rrole[rnum].MPType of
    0:
    begin
      MixColor2 := SDL_MapRGB(screen.format, 125, 205, 242);
      MixAlpha2 := 30 * Rrole[rnum].CurrentMP div MAX_MP;
    end;
    1:
    begin
      MixColor2 := SDL_MapRGB(screen.format, 243, 151, 37);
      MixAlpha2 := 30 * Rrole[rnum].CurrentMP div MAX_MP;
    end;
    2:
    begin
  ...
  begin
    MixColor2 := SDL_MapRGB(screen.format, 0, 114, 47);
    MixAlpha2 := Rrole[rnum].AttPoi div 2;
  end;
  //MixAlpha2 := 40;
  ...
  begin
    ...
    begin
      EPicAmount[enum] := LoadPNGTiles(formatfloat('resource/eft/eft000', enum), EPNGIndex[enum], EPNGTile[enum], 1);
      EPicLoaded[enum] := 1;
    end;
  ...
  begin
    CheckBasicEvent;
    DrawBFieldWithEft(i, beginpic, endpic, min, bnum, aimMode, mode, $FFFFFFFF, enum, MixColor2, MixAlpha2);
    SDL_UpdateRect2(screen, 0, 0, screen.w, screen.h);
    SDL_Delay(BATTLE_SPEED);
    i := i + 1;
...
begin
  ...
  begin
    ...
    begin
      ...
      str := format('EXP+%d', [basicvalue]);
      DrawEngShadowText(@str[1], x + 170, y + 75, ColColor($64), ColColor($66));
      p := p + 1;
    end;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            DrawRectangle(screen, CENTER_X - 150 + 30, CENTER_Y - 240 + 170,{115, 63,} 90, 25, 0, ColColor(255), 25);
            str := '製得物品';
            DrawShadowText(@str[1], CENTER_X - 150 + 15, CENTER_Y - 240 + 172, ColColor($21), ColColor($23));
...
procedure Rest(bnum: integer);
var
  rnum, i, j, curehurt, curepoison, neinum, neilevel: integer;
begin
  case Brole[bnum].Acted of
    2:
    begin
      rnum := Brole[bnum].rnum;
...
var
  ...
  begin
    ...
    begin
      if (i = menu) then
      begin
        DrawShadowText(@namestr[i][1], x + 13, y + 3 + 22 * i, ColColor($64), ColColor($66));
        DrawShadowText(@modestring[Brole[a[i]].AutoMode][1], x + 103, y + 3 + 22 * i,
          ColColor($64), ColColor($66));
      end
      else
      begin
        DrawShadowText(@namestr[i][1], x + 13, y + 3 + 22 * i, ColColor($21), ColColor($23));
        DrawShadowText(@modestring[Brole[a[i]].AutoMode][1], x + 103, y + 3 + 22 * i,
          ColColor($21), ColColor($23));
      end;
    end;
    if menu = -2 then
      DrawShadowText(@str[1], x + 13, y + 3 + 22 * amount, ColColor($64), ColColor($66))
    else
      DrawShadowText(@str[1], x + 13, y + 3 + 22 * amount, ColColor($21), ColColor($23));
...
begin
  ...
  begin
    ...
    //If all other actions fail, rest.
    //如果上面行动全部失败, 生命和内力都多于一半就向敌人移动, 如果有一个小于1/10则远离,否则原地调息
    if Brole[bnum].Acted <> 1 then
    begin
      if (Rrole[rnum].CurrentHP > Rrole[rnum].MaxHP div 2) and (Rrole[rnum].CurrentMP >
      ...
      else if (Rrole[rnum].CurrentHP < Rrole[rnum].MaxHP div 10) or (Rrole[rnum].CurrentMP <
...
var
  ...
  pFunc: function(bnum, mnum, level: integer): integer of object;
  f, i: integer;
  p1: array[0..9] of integer = (20, 20, 30, 20, 30, 20, 40, 30, 40, 20);
  p2: array[100..101] of integer = (15, 15);
begin
  f := Rrole[Brole[bnum].rnum].AmiFrameNum[0];
  if (f >= low(p1)) and (f <= high(p1)) then
    if random(100) < p1[f] then
    begin
  ...
  begin
    ...
    begin
      ...
      if (f >= low(p2)) and (f <= high(p2)) then
        if random(100) < p2[f] then
        begin
          Funcname := Format('SA2_%d', [f]);
          TMethod(pFunc).Code := TSpecialAbility2.MethodAddress(Funcname);
          if Assigned(TMethod(pFunc).Code) then
...
begin
  ...
  begin
    ...
    if AI then
      ...
      while res = -1 do
        res := CommonScrollMenu(CENTER_X - 50 - length(menuString[0]) * 5, 130, 105 +
