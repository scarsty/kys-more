...
{$ENDIF}
  ...
  //myInfo := SDL_GetVideoInfo()^;
  //writeln(myInfo.current_w,',',myInfo.blit_hw, ',',myInfo.blit_hw);
  ...
  //SDL_SetColorKey(ImgScence, SDL_SRCCOLORKEY, 1);
  //SDL_SetColorKey(ImgScenceBack, SDL_SRCCOLORKEY, 1);
  setlength(BlockImg, ImageWidth * ImageHeight);
  setlength(BlockImg2, ImageWidth * ImageHeight);
  for i := 0 to 5 do
    SimpleStatus[i] := SDL_CreateRGBSurface(ScreenFlag or SDL_SRCALPHA, 270, 100, 32, RMask, GMask, BMask, AMask);

  if Text_Layer = 1 then
  begin
    TextScreen := SDL_CreateRGBSurface(ScreenFlag or SDL_SRCALPHA, RESOLUTIONX, RESOLUTIONY,
...
begin
  ...
  begin
    ...
    //LoadPNGTiles('resource/head', HPNGIndex, HPNGTile, 1);
  end;
  ReadingTiles := True;
  now1 := SDL_GetTicks;
  ...
  begin
    LoadThread := SDL_CreateThread(@ReadTiles, nil);
    if (OPEN_MOVIE = 1) and (THREAD_READ_PNG = 1) then
  ...
  begin
    ...
    Redraw;
    str := '請稍候...';
    DrawShadowText(@str[1], CENTER_X - 40, CENTER_Y, ColColor($14), ColColor(7));
    UpdateAllScreen;
    ReadTiles;
  end;
  if IsConsole then
  ...
  begin
    ...
    for i := 0 to 2 do
    begin
      DrawTitlePic(16, CENTER_X + 50, CENTER_Y + i * 50, 0, 25, 0, 0);
      if i <> menu then
        DrawTitlePic(3 + i, x, y + 50 * i)
      else
        DrawTitlePic(6 + i, x, y + 50 * i);
    end;
    DrawTitlePic(13, CENTER_X - 320, CENTER_Y - 60, 0, 25 + alpha div 2, 0, 0);
...
begin
  ...
  begin
    Redraw;
    DrawTitlePic(9, x, y, 0, 100 - i * 5, 0, 0);
    UpdateAllScreen;
    SDL_Delay(20);
  ...
  temp := CopyIndexSurface(TitlePngIndex, 12);
  src.x := 0;
  src.y := 0;
  src.h := temp.h;
  dest.x := x;
  ...
  begin
    Redraw;
    DrawTitlePic(14 + i, CENTER_X + 50, CENTER_Y, 0, 75);
    DrawTitlePic(14 + i, CENTER_X + 50, CENTER_Y + 50, 0, 50);
    DrawTitlePic(14 + i, CENTER_X + 50, CENTER_Y + 100, 0, 25);
    UpdateAllScreen;
    SDL_Delay(20);
    if SDL_PollEvent(@event) >= 0 then
    begin
  ...
  begin
    ...
    for j := 0 to 2 do
    begin
      DrawTitlePic(16, CENTER_X + 50, CENTER_Y + j * 50, 0, 25);
      DrawTitlePic(3 + j, CENTER_X + 90, CENTER_Y + j * 50, 0, 100 - i * 5, 0, 0);
    end;
    UpdateAllScreen;
    SDL_Delay(20);
    if SDL_PollEvent(@event) >= 0 then
...
begin
  ...
  begin
    ...
    begin
      ...
      DrawTitlePic(18, -TitlePNGIndex[18].CurPointer^.w + CENTER_X * 2, 5, 0, i * 5);
      UpdateAllScreen;
      SDL_Delay(20);
      SDL_PollEvent(@event);
      {if ((event.key.keysym.sym = SDLK_ESCAPE) or (event.button.button = SDL_BUTTON_RIGHT)) then
  ...
  PlayMP3(60, 0);
  ScrollTextAmi(words, 20, 18, 50, 1, CENTER_X - 270, 1, 200, 18, 0);
  PlayMP3(StartMusic, -1);
  words.Free;
...
{$ENDIF}
  ...
  try
    ...
    PNG_LOAD_ALL := Kys_ini.ReadInteger('system', 'PNG_LOAD_ALL', 1);
    KEEP_SCREEN_RATIO := Kys_ini.ReadInteger('system', 'KEEP_SCREEN_RATIO', 1);
    //软件模式下禁止文字分层, 错误过多
    Text_Layer := Kys_ini.ReadInteger('system', 'Text_Layer', 0) and GLHR and KEEP_SCREEN_RATIO;
    ZIP_SAVE := Kys_ini.ReadInteger('system', 'ZIP_SAVE', 1);
    OPEN_MOVIE := Kys_ini.ReadInteger('system', 'OPEN_MOVIE', 1);
    THREAD_READ_PNG := Kys_ini.ReadInteger('system', 'THREAD_READ_PNG', 1);
...
var
  ...
  x, y, walking, speed, Mx1, My1, Mx2, My2, i, i1, i2, stillcount, x1, y1: integer;
  axp, ayp, axp1, ayp1, gotoEntrance, minstep, step, drawed: integer;
...
begin
  ...
  begin
    ...
    if (walking or speed = 0) and (where = 1) then
    begin
      if SCENCEAMI > 0 then
      begin
        //DrawScence;
...
begin
  ...
  DrawTextWithRect(@scencename[1], CENTER_X - DrawLength(scencename) * 5 - 3, 100,
    DrawLength(scencename) * 10 + 6, ColColor(5), ColColor(7));
  SDL_Delay(500);
...
function CommonMenu(x, y, w, max, default: integer; menuString, menuEngString: array of WideString): integer; overload;
begin
  Result := CommonMenu(x, y, w, max, default, menuString, menuEngString, 1, ColColor($5),
    ColColor($7), ColColor($64), ColColor($66));
end;
  ...
  begin
    ...
    case event.type_ of
      ...
      begin
        ...
        if (event.button.button = SDL_BUTTON_LEFT) then
        begin
          if MouseInRegion(x, y, w, max * 22 + 29) then
          begin
            Result := menu;
            break;
          end;
...
procedure ShowCommonMenu(x, y, w, max, menu: integer; menuString, menuEngString: array of WideString); overload;
begin
  ShowCommonMenu(x, y, w, max, menu, menuString, menuEngString, 1, ColColor($5), ColColor($7),
...
begin
  ...
  for i := 0 to min(max, length(menuString) - 1) do
    if i = menu then
    begin
      DrawShadowText(@menuString[i][1], x + 3, y + 2 + 22 * i, menucolor1, menucolor2);
      if p = 1 then
        DrawEngShadowText(@menuEngString[i][1], x + 3 + (len - 1) * 10, y + 2 + 22 * i, menucolor1, menucolor2);
    end
    else
    begin
      DrawShadowText(@menuString[i][1], x + 3, y + 2 + 22 * i, color1, color2);
      if p = 1 then
        DrawEngShadowText(@menuEngString[i][1], x + 3 + (len - 1) * 10, y + 2 + 22 * i, color1, color2);
    end;
...
function CommonScrollMenu(x, y, w, max, maxshow: integer; menuString, menuEngString: array of WideString): integer;
  overload;
var
  menu, menup, menutop, x1, y1: integer;
begin
  menu := 0;
  menutop := 0;
  ...
  //SDL_EnableKeyRepeat(0,10);
  //DrawMMap;
  RecordFreshScreen(x, y, w + 1, maxshow * 22 + 29);
...
begin
  ...
  for i := menutop to menutop + maxshow - 1 do
    ...
    begin
      DrawShadowText(@menuString[i][1], x + 3, y + 2 + 22 * (i - menutop), ColColor($66), ColColor($64));
      if p = 1 then
        DrawEngShadowText(@menuEngString[i][1], x + 93, y + 2 + 22 * (i - menutop), ColColor($66), ColColor($64));
    end
    else
    begin
      DrawShadowText(@menuString[i][1], x + 3, y + 2 + 22 * (i - menutop), ColColor($7), ColColor($5));
      if p = 1 then
...
begin
  ...
  //SDL_EnableKeyRepeat(0,10);
  //DrawMMap;
  RecordFreshScreen(x, y, w * (max + 1) + 1, 27);
  ShowCommonMenu2(x, y, w, menu, menuString, max);
  SDL_UpdateRect2(screen, x, y, w * (max + 1) + 1, 27);
  CleanKeyValue;
  while (SDL_WaitEvent(@event) >= 0) do
  begin
    CheckBasicEvent;
...
begin
  //redraw;
  LoadFreshScreen(x, y);
  DrawRectangle(screen, x, y, w * (max + 1), 26, 0, ColColor(255), 50);
  //if length(Menuengstring) > 0 then p := 1 else p := 0;
  for i := 0 to max do
    if i = menu then
    begin
      DrawShadowText(@menuString[i][1], x + 3 + i * w, y + 2, ColColor($66), ColColor($64));
    end
    else
    begin
      DrawShadowText(@menuString[i][1], x + 3 + i * w, y + 2, ColColor($7), ColColor($5));
    end;
...
begin
  ...
  DrawRectangle(screen, CENTER_X - 275, CENTER_Y - 160, 550, 310, 0, $FFFFFFFF, 50);
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            DrawSimpleStatusByTeam(i, pos[i].x, pos[i].y, SDL_MapRGB(screen.format, 192, 16, 56), 30);
          end;
        end
        else
        begin
...
begin
  ...
  rotoscr := CopyIndexSurface(MPNGIndex, 2021);
  CleanTextScreen;
  for i := DISABLE_MENU_AMI to 25 do
  begin
    SDL_BlitSurface(tempscr, nil, screen, nil);
    DrawMPic(2020, CENTER_X - 193, CENTER_Y - 182, 0, 100 - i * 4, 0, 0);
    for j := 0 to 3 do
    begin
      x1 := LinearInsert(i, 0, 25, CENTER_X, pos[j].x);
      y1 := LinearInsert(i, 0, 25, CENTER_Y, pos[j].y);
      DrawMPic(2021, x1 - 65, y1 - 70, 0, 100 - i * 4, 0, 0);
      DrawMPic(2022 + j, x1 - 30, y1 - 25, 0, 100 - i * 4, 0, 0);
    end;
    UpdateAllScreen;
    SDL_PollEvent(@event);
    CheckBasicEvent;
  ...
  begin
    ...
    begin
      ...
      begin
        temp := rotozoomSurface(rotoscr, -j * 6, 1, 0);
        j := j + 1;
        if j >= 60 then j := 0;
        dest.x := pos[i].x - temp.w div 2;
        dest.y := pos[i].y - temp.h div 2;
...
var
  ...
  begin
    ...
    DrawRectangle(screen, xp, yp, d * col + 7, 25, 0, ColColor(255), 30);
    DrawRectangle(screen, xp, 30 + yp, d * col + 7, d * row + 10, 0, ColColor(255), 30);
    DrawRectangle(screen, xp, 45 + dt + yp, d * col + 7, 25, 0, ColColor(255), 30);
    //i:=0;
    for i1 := 0 to row - 1 do
      ...
      begin
        ...
        begin
          ...
          if (item >= 0) then
          begin
            DrawIPic(item, i2 * d + 5 + xp, i1 * d + 35 + yp, 0, 10, 0, 20);
          end;
        end;
      end;
    ...
    begin
      ...
      //drawengtext(screen, @str[1], 631, 37, colcolor($64));
      DrawEngShadowText(@str[1], 330 + xp, 2 + yp, ColColor($64), ColColor($66));
      len := DrawLength(PChar(@Ritem[item].Name));
      //DrawU16text(screen, @RItem[item].Name, 496 - len * 5, 37, colcolor($21));
      DrawU16ShadowText(@Ritem[item].Name, 215 - len * 5 + xp, 2 + yp, ColColor($23), ColColor($21));
      ...
      begin
        str := '你的位置：';
        DrawShadowText(@str[1], 8 + xp, 47 + dt + yp, ColColor($21), ColColor($23));
        str := format('%3d,%3d', [My, Mx]);
        DrawEngShadowText(@str[1], 108 + xp, 47 + dt + yp, ColColor($64), ColColor($66));

        str := '船的位置：';
        DrawShadowText(@str[1], 188 + xp, 47 + dt + yp, ColColor($21), ColColor($23));
        str := format('%3d,%3d', [Shipx, shipy]);
        DrawEngShadowText(@str[1], 288 + xp, 47 + dt + yp, ColColor($64), ColColor($66));
      end
      else
      begin
        len := length(pWideChar(@Ritem[item].Introduction));
        DrawU16ShadowText(@Ritem[item].Introduction, 8 + xp, 47 + dt + yp, ColColor($5), ColColor($7));
        //如有人使用则显示
        if Ritem[item].User >= 0 then
        begin
          str := '使用';
          DrawShadowText(@str[1], 18 + length(PChar(@Rrole[Ritem[item].User].Name)) * 10 +
            len * 20 + xp, 47 + dt + yp, ColColor($21), ColColor($23));
          DrawU16ShadowText(@Rrole[Ritem[item].User].Name, 18 + len * 20 + xp, 47 + dt + yp,
            ColColor($64), ColColor($66));
        end;
      end;
    end;
    ...
    begin
      ...
      //yp := yp + y * d + 34 - dt;
      if len2 + len3 > 0 then
        DrawRectangle(screen, xp, 75 + dt + yp, d * col + 7, 20 * ((len2 + l1) div l + (len3 + l1) div l) + 7,
          0, ColColor(255), 30);
      //yp:=yp+5;
      //setfontsize(16, 15);
      if len2 > 0 then
      ...
      begin
        str := '需求：';
        DrawShadowText(@str[1], 8 + xp, 78 + dt + (len2 + l1) div l * 20 + yp, ColColor($21), ColColor($23));
      end;
      ...
      begin
        ...
        begin
          ...
          DrawShadowText(@words2[i][1], 68 + i1 mod l * w + xp, i1 div l * 20 + 78 + dt + yp,
            ColColor(5), ColColor(7));
          DrawShadowText(@str[1], 108 + i1 mod l * w + xp, i1 div l * 20 + 78 + dt + yp,
            color1, color2);
          i1 := i1 + 1;
        end;
      end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            //setfontsize(12, 19);
            str := '使用中';
            DrawTextWithRect(@str[1], CENTER_X - 384 + 30, CENTER_Y - 240 + 80 * i + 50, 67,
      ...
      DrawRectangle(screen, titlex1, titley1 - 1, d * col + 7, 25, 0, ColColor(255), 40);
      str := '·';
...
begin
  ...
  begin
    t := 250 - i * 2;
    color := SDL_MapRGB(screen.format, t, t, t);
...
procedure UseItem(inum: integer; teammate: integer = -1);
var
  x, y, menu, rnum, p, i, mask, potentialUser, preUser: integer;
  str, str1: WideString;
  menuString: array of WideString;
begin
  ...
  case Ritem[inum].ItemType of
    ...
    begin
      ...
      begin
        ...
        menuString[1] := '繼續';
        str := '此物品正有人裝備，是否繼續？';
        DrawTextWithRect(@str[1], CENTER_X - 142, CENTER_Y - 40, 285, ColColor(7), ColColor(5));
...
begin
  ...
  begin
    ...
    if ((event.type_ = SDL_KEYDOWN) and (event.key.keysym.sym = SDLK_UP)) or
      ((event.type_ = SDL_MOUSEBUTTONUP) and (event.button.button = sdl_button_wheelup)) then
    begin
      menu := menu - 1;
      if menu < 0 then
        menu := max;
    ...
    if ((event.type_ = SDL_KEYDOWN) and (event.key.keysym.sym = SDLK_DOWN)) or
      ((event.type_ = SDL_MOUSEBUTTONUP) and (event.button.button = sdl_button_wheeldown)) then
    begin
      menu := menu + 1;
      if menu > max then
        menu := 0;
    ...
    begin
      case CommonMenu2(item1x, item1y + d, 45, menuString, 2) of
        1:
        begin
          rnum := TeamList[menu];
    ...
    if ((event.type_ = SDL_MOUSEBUTTONUP) and (event.button.button = SDL_BUTTON_LEFT) and
      ...
      ((event.key.keysym.sym = SDLK_RETURN) or (event.key.keysym.sym = SDLK_SPACE))) then
    begin
      case CommonMenu2(item2x, item2y + d, 45, menuString, 2) of
        1:
        begin
          rnum := TeamList[menu];
...
begin
  ...
  begin
    //显示头像
    DrawHeadPic(Rrole[rnum].HeadNum, xp + 65, yp + 20);
    x := xp + 60;
    y := yp - 15;

    //显示姓名
    Name := pWideChar(@Rrole[rnum].Name);
    DrawShadowText(@Name[1], x + 88 - DrawLength(PChar(@Rrole[rnum].Name)) * 5, y + 200, ColColor($64), ColColor($66));
    //显示所需字符
    for i := 0 to 5 do
  ...
  begin
    if bnum = -1 then
    begin
      DrawRectangle(screen, x + 270, y, 260, 325, 0, ColColor(255), 50);
...
var
  ...
  head, smallhead, greenhead, sur, temp, ss, st: PSDL_Surface;
  dest, dest2, rectcut: TSDL_Rect;
  tran: byte;
  bigtran, Mask, mixColor: uint32;
  engsize, x1, y1, w1, h1, alpha: integer;
begin
  if rnum < 0 then exit;
  x1 := 0;
  y1 := 0;
  ...
  ss := SDL_CreateRGBSurface(screen.flags or SDL_SRCALPHA, 270, 90, 32, RMask, GMask, BMask, AMask);
  st := SDL_CreateRGBSurface(screen.flags or SDL_SRCALPHA, x1 + w1, y1 + h1, 32, RMask, GMask, BMask, AMask);
  ...
  temp := CopyIndexSurface(MPNGIndex, 2002);
  SDL_SetAlpha(temp, 0, 255);
  SDL_BlitSurface(temp, nil, sur, nil);
  SDL_FreeSurface(temp);

  //head := LoadSurfaceFromFile(AppPath + 'resource/head/'+ IntToStr(Rrole[rnum].HeadNum) + '.png');
  //head := SDL_CreateRGBSurface(screen.flags or SDL_SRCALPHA, 164, 164, 32, RMask, GMask, BMask, AMask);
  //SDL_FillRect(head, nil, 0);
  if PNG_LOAD_ALL = 0 then
    if HPNGIndex[Rrole[rnum].HeadNum].Loaded = 0 then
      LoadOnePNGTile('resource/head', pHPic, Rrole[rnum].HeadNum, HPNGIndex[Rrole[rnum].HeadNum], @HPNGTile[0]);
  head := CopyIndexSurface(HPngindex, Rrole[rnum].HeadNum);
  //DrawHeadPic(Rrole[rnum].HeadNum, 0, 0, head);
  smallhead := zoomSurface(head, 0.5, 0.5, SMOOTH);
  ...
  begin
    greenhead := MixSurface(smallhead, SDL_MapRGB(sur.format, 50, 200, 50), Rrole[rnum].Poison, 1);
    SDL_BlitSurface(greenhead, nil, sur, @dest);
    SDL_FreeSurface(greenhead);
  end
  else
    SDL_BlitSurface(smallhead, nil, sur, @dest);

  SDL_FreeSurface(smallhead);
  ...
  color := SDL_MapRGB(screen.format, 196, max(0, 25 - Rrole[rnum].Hurt div 5), 16);
  DrawRectangleWithoutFrame(sur, x + 96, y + 32, w, 9, color, -1);
  DrawRectangleWithoutFrame(sur, x + 96 + w, y + 32, 138 - w, 9, color * Text_Layer, alpha);

  w := 138 * Rrole[rnum].CurrentMP div Rrole[rnum].MaxMP;
  case Rrole[rnum].MPType of
    0: color := SDL_MapRGB(screen.format, 112, 12, 112);
    1: color := SDL_MapRGB(screen.format, 224, 180, 32);
    else
      color := SDL_MapRGB(screen.format, 160, 160, 160);
  end;
  DrawRectangleWithoutFrame(sur, x + 96, y + 48, w, 9, color, -1);
  DrawRectangleWithoutFrame(sur, x + 96 + w, y + 48, 138 - w, 9, color * Text_Layer, alpha);

  w := 83 * Rrole[rnum].PhyPower div MAX_PHYSICAL_POWER;
  color := SDL_MapRGB(screen.format, 128, 128, 255);

  DrawRectangleWithoutFrame(sur, x + 115, y + 65, w, 9, color, -1);
  DrawRectangleWithoutFrame(sur, x + 115 + w, y + 65, 83 - w, 9, color * Text_Layer, alpha);
  ...
  DrawShadowText(@str[1], x + 115, y + 9, ColColor($64), ColColor($66), sur);

  str := format('%d', [Rrole[rnum].Level]);
  DrawEngShadowText(@str[1], x + 103 - length(str) * 3, y + 6, ColColor(5), ColColor(7), sur);
  ...
  str := format('%4d', [Rrole[rnum].CurrentHP]);
  DrawEngShadowText(@str[1], x + 138, y + 27, color1, color2, sur);

  str := '/';
  DrawEngShadowText(@str[1], x + 165, y + 27, ColColor($64), ColColor($66), sur);
  ...
  str := format('%4d', [Rrole[rnum].MaxHP]);
  DrawEngShadowText(@str[1], x + 173, y + 27, color1, color2, sur);
  ...
  begin
    color1 := ColColor($50);
    color2 := ColColor($4E);
    color := SDL_MapRGB(screen.format, 112, 12, 112);
  end
  else
  ...
  begin
    color1 := ColColor($1D);
    color2 := ColColor($1C);
    color := SDL_MapRGB(screen.format, 224, 180, 32);
  end
  else
  begin
    color1 := ColColor($66);
  ...
  DrawEngShadowText(@str[1], x + 138, y + 44, color1, color2, sur);

  str := '/';
  DrawEngShadowText(@str[1], x + 165, y + 44, color1, color2, sur);

  str := format('%4d', [Rrole[rnum].MaxMP]);
  DrawEngShadowText(@str[1], x + 173, y + 44, color1, color2, sur);


  str := format('%3d', [Rrole[rnum].PhyPower]);
  DrawEngShadowText(@str[1], x + 148, y + 61, ColColor(5), ColColor(7), sur);
  ...
  if ForTeam = -1 then
  begin
    SDL_BlitSurface(ss, nil, screen, @dest2);
    if Text_Layer = 1 then
    begin
      ...
      dest2.x := dest2.x;
      dest2.y := dest2.y;
      SDL_SetColorKey(st, SDL_SRCCOLORKEY, 0);
...
begin
  ...
  g2 := trunc(g1 * 0.8);
  b2 := trunc(b1 * 0.8);
  color1 := SDL_MapRGB(screen.format, r1, g1, b1);
  color2 := SDL_MapRGB(screen.format, r2, g2, b2);
end;
...
begin
  ...
  begin
    ...
    if ((event.type_ = SDL_KEYUP) and ((event.key.keysym.sym = SDLK_RETURN) or
      (event.key.keysym.sym = SDLK_SPACE))) or ((event.type_ = SDL_MOUSEBUTTONUP) and
      (event.button.button = SDL_BUTTON_LEFT) and (mouseactive = 1)) then
    begin
      if select >= 0 then
      begin
        ...
        if select = 2 then
        begin
          case CommonMenu(itemx + d + 10, itemy, 45, 2, 2, menustring2) of
            0:
            begin
              MenuItemType := 4;
              MenuEscType := 2;
...
begin
  ...
  //menu := SelectOneTeamMember(80, 65, '%3d', 15, 0);
  TransBlackScreen;
  menu := SelectOneTeamMember(CENTER_X - 384 + 270 + 50, CENTER_Y - 240 + 5 + 80, UTF8Decode('要求誰離隊'), 0, 0);
  if menu >= 0 then
  begin
    for i := 0 to 99 do
...
procedure MenuSet;
var
  menu, pmenu, x, y, w, h, i, valuechanged, maxmenu, pressed, leftright: integer;
  filename: string;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          DrawMPic(2004, arrowlx, y + 5 + i * 25 + arrowy, 0, 0, mixcolorl, mixalphal);
          DrawMPic(2005, arrowrx, y + 5 + i * 25 + arrowy, 0, 0, mixcolorr, mixalphar);
        end
        else
        begin
          if i = 5 then
        ...
        DrawShadowText(puint16(str[i]), x + 10, y + 5 + i * 25, color1, color2);
        DrawShadowText(puint16(str2[i]), x + 170, y + 5 + i * 25, color1, color2);
      end;
      for i := 0 to 1 do
      begin
        ...
        DrawShadowText(puint16(menuString[i]), x + 170 + 50 * i, y + 5 + maxmenu * 25, color1, color2);
      end;
    ...
    case event.type_ of
      ...
      begin
        case event.button.button of
          ...
          begin
            ...
            begin
              if MouseInRegion(x + 160 + 13, y + 5 + maxmenu * 25, 50, 25) or
                MouseInRegion(x + 210 + 13, y + 5 + maxmenu * 25, 50, 25) then
              begin
                pressed := Value[maxmenu];
                //writeln(pressed);
                break;
              end;
              if MouseInRegion(x + 160 + 13, y + 5 + 5 * 25, 50, 25) then
              begin
                Value[5] := 1 - Value[5];
                //valuechanged := 1;
              end;
              if MouseInRegion(x + 160 + 13, y + 5 + 6 * 25, 50, 25) then
              begin
                Value[6] := 1 - Value[6];
                //valuechanged := 1;
              end;
              if MouseInRegion(x + 160 + 13, y + 5 + 7 * 25, 50, 25) then
              begin
                Value[7] := 1 - Value[7];
                //valuechanged := 1;
              end;
      ...
      begin
        if MouseInRegion(x, y + 5, w, h - 5) then
        begin
          menu := min((round(event.button.y * screen.h / RealScreen.h) - y - 5) div 25, maxmenu);
          if menu = maxmenu then
          begin
            if MouseInRegion(x + 160 + 13, y + 5 + maxmenu * 25, 50, 25) then
            begin
              Value[maxmenu] := 0;
              valuechanged := 1;
            end;
            if MouseInRegion(x + 210 + 13, y + 5 + maxmenu * 25, 50, 25) then
            begin
              Value[maxmenu] := 1;
              valuechanged := 1;
            end;
  ...
  begin
    ...
    begin
      FULLSCREEN := Value[6];
      RealScreen := SDL_SetVideoMode(CENTER_X * 2, CENTER_Y * 2, 32, ScreenFlag or
        (SDL_FULLSCREEN * sign(FULLSCREEN)));
    end;
...
begin
  ...
  menu := CommonMenu(CENTER_X - 384 + 420, CENTER_Y - 240 + 90, 280, 10, 0, menuString, menuEngString);
  if menu >= 0 then
  begin
    LastShowScene := -1;
    ...
    begin
      menu := -1;
      str := '讀檔失敗！索引丟失或文件不存在！';
      DrawTextWithRect(@str[1], CENTER_X - 384 + 420, CENTER_Y - 240 + 90 + 260, 322, ColColor($5), ColColor($7));
      WaitAnyKey;
    end;
    //SDL_UpdateRect2(screen, 0, 0, screen.w, screen.h);
  end;
...
begin
  ...
  begin
    ...
    color1 := 0;
    color2 := $20202020;
    menucolor1 := SDL_MapRGB(screen.format, 240, 20, 20);
    menucolor2 := SDL_MapRGB(screen.format, 212, 20, 20);
  end;
  //setlength(menuengstring, 0);
  menuString[0] := '進度一';
  menuString[1] := '進度二';
  ...
  DrawTitlePic(17, CENTER_X - 80, CENTER_Y - 70, 0, 0, 0, mixAlpha);
  SDL_UpdateRect2(screen, 0, 0, screen.w, screen.h);
  menu := CommonMenu(CENTER_X - 45, CENTER_Y - 50, 300, 10, 0, menuString, menuEngString,
    0, color1, color2, menucolor1, menucolor2);
  if menu >= 0 then
  begin
    if LoadR(menu + 1) then
    begin
...
begin
  ...
  begin
    ...
    if menu >= 0 then
      ...
      begin
        str := '存檔失敗！索引丟失或文件被佔用！';
        DrawTextWithRect(@str[1], CENTER_X - 384 + 420, CENTER_Y - 240 + 90 + 240, 322, ColColor($5), ColColor($7));
        WaitAnyKey;
      end;
  end;
  //ShowMenu(3);
...
begin
  ...
  if KDEF_SCRIPT > 0 then
    n := 2;
  menu := CommonMenu(CENTER_X - 384 + 660, CENTER_Y - 240 + 90, 45, n, 0, menuString);
  if menu = 1 then
  begin
    where := 3;
    instruct_14;
    ...
    //WaitAnyKey;
    //Redraw;
    //DrawTitlePic(0, TitlePosition.x, TitlePosition.y);
    //SDL_UpdateRect2(screen, 0, 0, screen.w, screen.h);
  end;
  ...
  begin
    ...
    str := '輸入腳本文件名';
    DrawTextWithRect(@str[1], CENTER_X - 80, CENTER_Y - 240 + 130, 145, ColColor($21), ColColor($27));
    str1 := IntToStr(EnterNumber(0, 100, CENTER_X - 80, CENTER_Y - 240 + 200, 1));
    str := '輸入功能編號';
    DrawTextWithRect(@str[1], CENTER_X + 120, CENTER_Y - 240 + 130, 125, ColColor($21), ColColor($27));
    str2 := 'f' + IntToStr(EnterNumber(0, 32767, CENTER_X + 120, CENTER_Y - 240 + 200, 0));
    if ExecScript(AppPath + 'script/' + str1 + '.lua', str2) <> 0 then
    begin
      str := ' Script fail!';
      DrawTextWithRect(@str[1], CENTER_X - 384 + 400, CENTER_Y - 240 + 150, 150, $FFFFFF, $FFFFFF);
      WaitAnyKey;
    end;
  end;
  //if menu <> 1 then
...
begin
  ...
  begin
    TransBlackScreen;
    DrawRectangle(screen, CENTER_X - 150 + 30, 170, {115, 98,} 155, 51, 0, ColColor(255), 30);
    DrawU16ShadowText(@Rrole[role2].Name, CENTER_X - 150 + 15, 172, ColColor($23), ColColor($21));
    word := ('增加生命');
    DrawShadowText(@word[1], CENTER_X - 150 + 15, 197, ColColor($7), ColColor($5));
...
begin
  ...
  begin
    TransBlackScreen;
    DrawRectangle(screen, CENTER_X - 150 + 30, 170, 155, 51, 0, ColColor(255), 30);
    word := ('減少中毒');
    DrawShadowText(@word[1], CENTER_X - 150 + 15, 197, ColColor($7), ColColor($5));
    DrawU16ShadowText(@Rrole[role2].Name, CENTER_X - 150 + 15, 172, ColColor($23), ColColor($21));
...
begin
  ...
  DrawRectangle(screen, 30 + xp, 100 + yp, 200, 25, 0, ColColor(255), 25);
  str := '服用';
  if Ritem[inum].ItemType = 2 then
    str := '練成';
  DrawShadowText(@str[1], 33 + xp, 102 + yp, ColColor($21), ColColor($23));
  ...
  begin
    l := p;
    DrawRectangle(screen, 30 + xp, 130 + yp, 200, 22 * l + 25, 0, ColColor($FF), 25);
  end
  else
  begin
    l := p div 2 + 1;
    xp := xp - 90;
    DrawRectangle(screen, 30 + xp, 130 + yp, 400, 22 * l + 25, 0, ColColor($FF), 25);
  end;
  DrawU16ShadowText(@Rrole[rnum].Data[4], 33 + xp, 132 + yp, ColColor($21), ColColor($23));
  str := '未增加屬性';
  if p = 0 then
    DrawShadowText(@str[1], 103 + xp, 132 + yp, ColColor(5), ColColor(7));
  p := 0;
  ...
  begin
    ...
    begin
      ...
      DrawShadowText(@word[i, 1], 33 + xp + x, 154 + yp + y + p * 22, ColColor(5), ColColor(7));
      if i <> 21 then
        str := format('%5d', [addvalue[i]])
      else
        str := format('%5.1f', [addvalue[i] / 10]);
      DrawEngShadowText(@str[1], 163 + xp + x, 154 + yp + y + p * 22, ColColor($64), ColColor($66));
      p := p + 1;
    end;
    //对内力性质特殊处理
...
begin
  //CallEvent(251);
  words := TStringList.Create;
  //words.LoadFromFile(AppPath + 'list/group.txt');
  words.Add('');
  words.Add('');
  words.Add('《金庸水滸傳》');
...
begin
  ...
  tempscr := SDL_CreateRGBSurface(ScreenFlag, CENTER_X * 2, (words.Count - 1) * linespace +
    CENTER_Y * 3, 32, 0, 0, 0, 0);
  //Redraw;
  if style <> 0 then
    SDL_FillRect(screen, nil, 0);
  if picnum < 0 then
  ...
  if style = 0 then
    SDL_SetAlpha(tempscr, SDL_SRCALPHA, 192);
  ...
  begin
    ...
    if align = 0 then
    begin
      x := CENTER_X - len * chnsize div 4 - 10;
    end
    else
    begin
      x := alignx;
    ...
    DrawShadowText(@str[1], x, l * linespace + CENTER_Y * 2, color1, color2, tempscr, 1);
  end;
  SetFontSize(20, 18);
  ...
  begin
    ...
    begin
      ...
      begin
        w := TitlePNGIndex[picnum].CurPointer^.w;
        h := TitlePNGIndex[picnum].CurPointer^.h;
        DrawTitlePic(picnum, -trunc(i * (w - CENTER_X * 2) / (tempscr.h - CENTER_Y * 2)) - w + CENTER_X * 2, 5);
        dest.y := CENTER_Y + 10;
        tempdest.x := 0;
        tempdest.y := -i + CENTER_Y;
        tempdest.w := CENTER_X * 2;
        tempdest.h := CENTER_Y - 20;
        SDL_BlitSurface(tempscr, @tempdest, screen, @dest);
      end;
    end;
    SDL_UpdateRect2(screen, 0, 0, screen.w, screen.h);
    i := i - 2;
    if i <= -tempscr.h + CENTER_Y * 2 then
    begin
      WaitAnyKey;
      break;
    end;
...
begin
  //CHINESE_FONT := pansichar(GetEnvironmentVariable('WINDIR') + '/fonts/kaiu.ttf');
  if force <> -1 then
  begin
    TTF_CloseFont(font);
    TTF_CloseFont(engfont);
...
begin
  for i := 0 to 5 do
  begin
    SDL_FillRect(SimpleStatus[i], nil, 0);
    if (Text_Layer = 1) then
...
begin
  ...
  //dest2.x := dest2.x - x;
  //dest2.y := dest2.y - y;
  tempsur := MixSurface(SimpleStatus[i], mixColor, mixAlpha);
  SDL_BlitSurface(tempsur, nil, screen, @dest);
  SDL_FreeSurface(tempsur);
  if (Text_Layer = 1) then
  begin
    if mixColor = 0 then
    begin
      tempsur := MixSurface(SimpleText[i], mixColor, mixAlpha);
      SDL_SetAlpha(tempsur, 0, 255);
      SDL_BlitSurface(tempsur, @rectcut, TextScreen, @dest2);
      SDL_FreeSurface(tempsur);
    end
    else
...
begin
  ...
  if Text_Layer = 1 then
  begin
    DrawRectangleWithoutFrame(TextScreen, 0, 0, TextScreen.w, TextScreen.h, 0, 50);
  end;
end;
...
begin
  ...
  dest.w := w;
  dest.h := h;
  //if freshscreen <> nil then SDL_FreeSurface(freshscreen);
  //freshscreen := SDL_CreateRGBSurface(screen.flags, w, h, 32, RMask, GMask, BMask, 0);
  //SDL_BlitSurface(screen, @dest, freshscreen, nil);
  i := length(freshscreen);
  setlength(freshscreen, i + 1);
  freshscreen[i] := SDL_CreateRGBSurface(screen.flags, w, h, 32, RMask, GMask, BMask, 0);
  SDL_BlitSurface(screen, @dest, freshscreen[i], nil);
  CleanTextScreenRect(x, y, w, h);
  if IsConsole then
...
begin
  ...
  begin
    dest.x := x;
    dest.y := y;
    SDL_BlitSurface(freshscreen[i], nil, screen, @dest);
    CleanTextScreenRect(x, y, freshscreen[i].w, freshscreen[i].h);
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      SDL_FillRect(TextScreen, @dest, 0);
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    w := round(w * scale);
    h := round(h * scale);
    //x := round(x * realscreen.w/screen.w);
    //y := round(y * realscreen.h/screen.h);
    //w := round(w * realscreen.w/screen.w);
    //h := round(h * realscreen.h/screen.h);
  end;
  Result.x := x;
  Result.y := y;
  Result.w := w;
