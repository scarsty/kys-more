...
begin
  ...
  DrawRectangle(screen, x, y - 2, DrawLength(str) * 10 + 5, 26, incolor, framecolor, alpha);
...
var
  ...
  str: WideString = '';
begin
  talknum := lua_tointeger(L, -2);
  if not lua_isnumber(L, -2) then
    str := UTF8Decode(lua_tostring(L, -2));
  NewTalk(0, talknum, -1, 2, 1, lua_tointeger(L, -1), 0, str);
end;
