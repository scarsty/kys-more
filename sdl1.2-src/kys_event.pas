...
begin
  ...
  if where = 2 then
  begin
    x := CENTER_X - 150 + 30 + 85;
    y := CENTER_Y - 240 + 200;
  end;

  DrawRectangle(screen, x - 85, y, 170, 76, 0, ColColor(255), 30);
  if amount >= 0 then
    word := '得到物品'
  else
  begin
  ...
  DrawShadowText(@word[1], x - 80, 2 + y, ColColor($21), ColColor($23));
  DrawU16ShadowText(@Ritem[inum].Name, x - 80, 27 + y, ColColor(5), ColColor(7));
  word := '數量';
  DrawShadowText(@word[1], x - 80, 52 + y, ColColor($64), ColColor($66));
  word := format('%5d', [amount]);
  DrawEngShadowText(@word[1], x - 5, 52 + y, ColColor($64), ColColor($66));
  UpdateAllScreen;
...
begin
  where := 3;
  Redraw;
  DrawRectangleWithoutFrame(screen, 0, 0, CENTER_X * 2, CENTER_Y * 2, SDL_MapRGB(screen.format, 196, 25, 16), 60);
  str := ' 小村的傳說失敗了…';
...
begin
  ...
  begin
    ...
    word := '學會';
    DrawShadowText(@word[1], CENTER_X - 70, 125, ColColor(5), ColColor(7));
    DrawU16ShadowText(@Rrole[rnum].Name, CENTER_X - 70, 100, ColColor($21), ColColor($23));
    DrawU16ShadowText(@Rmagic[mnum].Name, CENTER_X - 70, 150, ColColor($64), ColColor($66));
    SDL_UpdateRect2(screen, 0, 0, screen.w, screen.h);
    WaitAnyKey;
    Redraw;
  end;
...
begin
  ...
  if word <> '' then
  begin
    DrawRectangle(screen, CENTER_X - 75, 98, 150, 51, 0, ColColor(255), 30);
    DrawShadowText(@word[1], CENTER_X - 70, 125, ColColor(5), ColColor(7));
    DrawU16ShadowText(@Rrole[rnum].Name, CENTER_X - 70, 100, ColColor($21), ColColor($23));
    word := format('%4d', [Value]);
...
begin
  ...
  DrawShadowText(@word[1], CENTER_X - 105, 100, ColColor(5), ColColor(7));
  word := format('%3d', [Rrole[rnum].Data[datalist]]);
...
begin
  ...
  for i := CENTER_Y * 2 - TitlePNGIndex[1].CurPointer^.h to 0 do
  begin
    if i mod 5 = 0 then
      DrawTitlePic(1, CENTER_X - 320, i);
    SDL_UpdateRect2(screen, 0, 0, screen.w, screen.h);
    SDL_Delay(6);
  end;
  SDL_Delay(1000);
  ...
  case code of
    ...
    begin
      ...
      begin
        ...
        begin
          pw^ := 0;
          DrawU16ShadowText(PChar(pw1), e3 - 2, e4 + 22 * i - 25, ColColor(e5 and $FF),
    ...
    begin
      ...
      e5 := e_GetValue(3, e1, e5);
      e6 := e_GetValue(4, e1, e6);
      DrawRectangle(screen, e2, e3, e4, e5, 0, ColColor($FF), max(e6, 40));
    ...
    begin
      ...
      if (pw^) = $2A then
        i1 := i1 - 1;
      DrawRectangle(screen, e3, e4, i2 * 20 + 25, i1 * 22 + 5, 0, ColColor(255), 30);
      pw := @x50[e2];
      pw1 := pw;
      i := 0;
      while uint16(pw^) > 0 do
...
procedure NewTalk(headnum, talknum, namenum, place, showhead, color, frame: integer;
  content: WideString = ''; disname: WideString = '');
var
  FileHandle, Offset, len, I, I2, ix, iy, xtemp: integer;
...
{$ENDIF}
  ...
  begin
    ...
    DrawRectangleWithoutFrame(screen, 0, Frame_Y, CENTER_X * 2, 170, 0, 50);
    //显示头像
...
begin
  ...
  DrawRectangle(screen, x, y, 200, 200, 0, ColColor($FF), 40);
  DrawRectangle(screen, x, y - 30, 120, 30, 0, ColColor($FF), 40);
  DrawRectangle(screen, x - 32, y - 30, 32, 230, 0, ColColor($FF), 40);
...
var
  ...
  CurrentStar, CurrentTeam, page, numStar, state, headn, menup, menu, x1, y1: integer;
...
begin
  ...
  begin
    ...
    begin
      refresh := False;
      LoadFreshScreen;
      DrawRectangle(screen, xStar, yStar, 200, 18 * 22 + 8, 0, ColColor(255), 50);
      for i := 0 to 17 do
        if (i = numStar) and (menuid = 0) then
        begin
          DrawShadowText(@StarMenu[i][1], xStar + 3, yStar + 2 + 22 * i, ColColor($64), ColColor($66));
        end
        else if StateList[i] = 2 then
        begin
          DrawShadowText(@StarMenu[i][1], xStar + 3, yStar + 2 + 22 * i, ColColor($80), ColColor($82));
        end
        else if StateList[i] > 2 then
        begin
          DrawShadowText(@StarMenu[i][1], xStar + 3, yStar + 2 + 22 * i, ColColor($20), ColColor($22));
        end
        else
          DrawShadowText(@StarMenu[i][1], xStar + 3, yStar + 2 + 22 * i, ColColor($5), ColColor($7));

      DrawRectangle(screen, xTeam, yTeam, 200, 28, 0, ColColor(255), 50);
      DrawShadowText(@TeamMenu[0][1], xTeam + 3, yTeam + 2, ColColor($5), ColColor($7));

      DrawRectangle(screen, xTeam, yTeam + 38, 200, 4 * 22 + 28, 0, ColColor(255), 50);
      for i := 1 to 5 do
        if (i = CurrentTeam) and (menuid = 1) then
        begin
          DrawShadowText(@TeamMenu[i][1], xTeam + 3, yTeam + 40 + 22 * (i - 1), ColColor($64), ColColor($66));
        end
        else
        begin
          DrawShadowText(@TeamMenu[i][1], xTeam + 3, yTeam + 40 + 22 * (i - 1), ColColor($5), ColColor($7));
        end;
      ...
      begin
        DrawRectangle(screen, xState, yState + 83, 200, 95, 0, ColColor(255), 50);
    ...
    begin
      ...
      case event.type_ of
        ...
        SDL_MOUSEMOTION:
        begin
          if MouseInRegion(xStar, yStar, 200, 18 * 22 + 29, x1, y1) then
          begin
            menuid := 0;
            menup := CurrentStar;
            menu := (y1 - yStar - 2) div 22;
            if menu > 17 then
              menu := 17;
            if menu < 0 then
              menu := 0;
            CurrentStar := menu + CurrentStar div 18 * 18;
            refresh := menup <> CurrentStar;
            break;
          end;
          if MouseInRegion(xTeam, yTeam, 200, 5 * 22 + 29, x1, y1) then
          begin
            menuid := 1;
            menup := CurrentTeam;
            menu := (y1 - yTeam - 40) div 22 + 1;
            if menu > 5 then
              menu := 5;
            if menu < 0 then
              menu := 0;
            CurrentTeam := menu;
            refresh := menup <> CurrentTeam;
            break;
          end;
        end;
      end;
...
begin
  ...
  hw := 57;
  hh := 72;
  DrawRectangleWithoutFrame(screen, 0, 40, screen.w, 230, 0, 40);
...
var
  ...
  begin
    ...
    for i := menutop to menutop + maxshow - 1 do
      if i = menu then
      begin
        DrawShadowText(@menuString[i][1], x + 3, y + 2 + 22 * (i - menutop), ColColor($64), ColColor($66));
      end
      else
      begin
        DrawShadowText(@menuString[i][1], x + 3, y + 2 + 22 * (i - menutop), ColColor($5), ColColor($7));
      end;
    ...
    begin
      ...
      hh := 72;
      i := 0;
      DrawRectangle(screen, CENTER_X - 384 + 304, y, 379, maxshow * 22 + 6, 0, ColColor(255), 75);
...
begin
  ...
  DrawRectangleWithoutFrame(screen, x - 10, y - 10, c * 50 + 20, r * 50 + 20, 0, 60);
  for i := 0 to c * r - 1 do
  begin
...
begin
  ...
  begin
    menu := CommonScrollMenu(x, y, 500, n - 1, 20, menuString);
    Redraw;
    UpdateAllScreen;
  end;
end;
...
begin
  ...
  DrawRectangleWithoutFrame(screen, framex, framey, w, h - 1, color, 50);
...
function SpellPicture(num, chance: integer): boolean;
var
  x, y, w, h, i, x1, y1, r, right, menu, menu1, menu2, picnum: integer;
  pic: PSDL_Surface;
  filename: string;
...
begin
  ...
  if SPNGIndex[picnum].Loaded = 0 then
    LoadOnePNGTile('resource/smap', pSPic, picnum, SPNGIndex[picnum], @SPNGTile[0]);
  Pic := CopyIndexSurface(SPNGIndex, picnum);
  ...
  DrawRectangle(screen, x - 5, y - 5, w, h, 0, ColColor(255), 50);
  for i := 0 to 24 do
  begin
  ...
  if menu2 > -1 then
    DrawRectangle(screen, (menu2 mod 5) * 80 + x, (menu2 div 5) * 80 + y + 30, 80, 80, 0, ColColor($64), 0);
  if menu > -1 then
    DrawRectangle(screen, (menu mod 5) * 80 + x, (menu div 5) * 80 + y + 30, 80, 80, 0, ColColor($255), 0);
...
begin
  ...
  i := CommonScrollMenu(x, y, w, j - 1, 20, menuString);
  if i > -1 then
    i := itemlist[i];
  x50[28931] := i;
...
begin
  ...
  iskey := True;
  DrawRectangle(screen, (320 - 43 * (DanceLong div 2)) - 5, 120 - 62, (DanceLong + 1) * 43 + 5, 45,
    ColColor(0), ColColor($255), 25);
  DrawRectangle(screen, 0, 420, 640, 14, ColColor(47), ColColor($255), 0);
  SDL_UpdateRect2(screen, 0, 0, screen.w, screen.h);
  ...
  begin
    ...
    if (now - ori_time) < (demand_time * 0.7) then
      DrawRectangle(screen, 0, 420, 640 * (now - ori_time) div demand_time, 14, ColColor(47), ColColor($255), 100)
    else if ((now - ori_time) >= (demand_time * 0.7)) and ((now - ori_time) <= (demand_time * 1)) then
      DrawRectangle(screen, 0, 420, 640 * (now - ori_time) div demand_time, 14, ColColor(70), ColColor($255), 100)
    else
    begin
      WaitAnyKey;
      //lose
...
var
  ...
  begin
    ...
    DrawRectangle(screen, x, y, 420, 116, 0, ColColor(255), 50);
    for i := 0 to 4 do
    begin
      menuEngString[i] := format('%10d%7d%6d%9d', [sell.Price[i], sell.Amount[i], buy.HoldAmount[i],
        buy.BuyAmount[i]]);
      mixcolorl := 0;
      mixcolorr := 0;
      mixalphal := 0;
      ...
      if i = menu then
      begin
        DrawShadowText(@menuString[i][1], x + 3, y + 2 + 22 * i, ColColor($66), ColColor($64));
        DrawEngShadowText(@menuEngString[i][1], x + 70, y + 2 + 22 * i, ColColor($66), ColColor($64));
        if lr < 0 then
        begin
          mixcolorl := $FFFFFFFF;
          mixalphal := 25;
      ...
      begin
        DrawShadowText(@menuString[i][1], x + 3, y + 2 + 22 * i, ColColor($7), ColColor($5));
        DrawEngShadowText(@menuEngString[i][1], x + 70, y + 2 + 22 * i, ColColor($7), ColColor($5));
      end;
      ...
      DrawMPic(2004, arrowlx, i * l + arrowy, 0, 0, mixcolorl, mixalphal);
      DrawMPic(2005, arrowrx, i * l + arrowy, 0, 0, mixcolorr, mixalphar);
    end;
    ...
    word[1] := '反悔';
    word[2] := '離開';
    DrawRectangle(screen, x2, y2, w2, 26, 0, ColColor(255), 50);
    for i := 0 to 2 do
...
begin
  ...
  arrowy := y + 5;
  l := 22;
  DrawRectangleWithoutFrame(screen, 0, 20, CENTER_X shl 1, 120, 0, 40);
  Shop_Str := '需要買什麽？';
...
begin
  ...
  if SPNGIndex[picnum].Loaded = 0 then
    LoadOnePNGTile('resource/smap', pSPic, picnum, SPNGIndex[picnum], @SPNGTile[0]);
  Map_Pic := CopyIndexSurface(SPNGIndex, picnum);
  l := scenceAmount;
  for i := 0 to l - 1 do
  begin
  ...
  begin
    ...
    begin
      ...
      SDL_BlitSurface(Map_Pic, @dest1, screen, @dest);
      SDL_BlitSurface(Map_Pic, @dest1, screen, @dest);
      DrawShadowText(@str2[p][1], 37, 80, ColColor(21), ColColor(25));
      DrawEngShadowText(@str3[p][1], 37, 100, ColColor(255), ColColor(254));
      DrawShadowText(@str[1], 37, 275, ColColor(21), ColColor(25));
      str1 := format('%3d, %3d', [My, Mx]);
      DrawEngShadowText(@str1[1], 37, 295, ColColor(255), ColColor(254));
      DrawShadowText(@strboat[1], 37, 325, ColColor(21), ColColor(25));
