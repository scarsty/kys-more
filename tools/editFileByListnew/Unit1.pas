...
var
  ...
  ch: Char;
  ch1: Byte;
  str, fileori, filebak: string;
  Value: Byte;
begin
  fileori := Edit1.Text;
  filebak := fileori+'.bak';
  CopyFile(pchar(fileori), pchar(filebak), false);
  FileZ := FileOpen(Edit1.Text, fmOpenReadWrite);
  FileL := FileOpen(Edit2.Text, fmOpenRead);
  Add := strtoint(edit3.Text);
  if (FileZ <> -1) and (FileL <> -1) then begin
    ...
    FileClose(FileZ);
    FileClose(FileL);
    Label3.Caption := inttostr(sum) + ' bytes modified.';
  end
  else
  begin
...
begin
  ...
  str := str + 'AddressShift=' + edit3.Text + ';' + char(13) + char(10);
